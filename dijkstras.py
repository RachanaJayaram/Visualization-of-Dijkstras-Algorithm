...
def dijkstra(graph,queue):
     ...
     print(queue.queue)
     while queue.length:
          step={"dist":dist.copy(),"min_dist_router":queue.queue[0][1],"queue":queue.queue[:],"adjacent_nodes":None,"adj_node":None,"old_dist":None,"dist_with":None,"new_dist":None,"settled":settled.copy()}
          ...
          for adjacent_node in graph.adjacency_list[min_dist_router].keys():
               ...
               if adjacent_node not in settled:
                    ...
                    if new_edge_distance<dist[adjacent_node]:
                         ...
                         if adjacent_node not in [node[1] for node in queue.queue]:
     ...
     step={"dist":dist.copy(),"min_dist_router":None,"queue":queue.queue[:],"adjacent_nodes":None,"adj_node":None,"old_dist":None,"dist_with":None,"new_dist":None,"settled":settled.copy()}
